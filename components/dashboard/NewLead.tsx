...
const NewLead: React.FC<NewLeadProps> = ({ onCreateLead, onCancel, onBack }) => {
    ...
    return (
        <div>
            <div>
                <button
                    onClick={onBack}
                    className="mb-5 flex items-center gap-2 text-slate-400 hover:text-[#1E3A6D] font-bold transition-colors group"
                >
            ...
            {showSummary && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl animate-[slideIn_0.3s_ease-out] overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <h3 className="text-xl font-extrabold text-[#1E3A6D]">Review Quotation Request</h3>
                        ...
                        <div className="p-6 overflow-y-auto space-y-6">
                            ...
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                <h4 className="text-sm font-bold text-[#1E3A6D] uppercase tracking-wider mb-3">Guest Information</h4>
                                <div className="grid grid-cols-2 gap-y-2 text-sm">
                                    <div className="text-slate-500">Full Name:</div>
                                    <div className="font-bold text-slate-700">{guestDetails.name}</div>
                                    <div className="text-slate-500">Email:</div>
                                    <div className="font-bold text-slate-700">{guestDetails.email}</div>
                                    <div className="text-slate-500">Phone:</div>
                                    <div className="font-bold text-slate-700">{guestDetails.phone}</div>
                                    <div className="text-slate-500">Travelers:</div>
                                    <div className="font-bold text-slate-700">
                                        {guestDetails.adults} Adults, {guestDetails.child} Children, {guestDetails.infant} Infants
                                        ({guestDetails.adults + guestDetails.child + guestDetails.infant} Total)
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                <h4 className="text-sm font-bold text-[#1E3A6D] uppercase tracking-wider mb-3">Trip Itinerary</h4>
                                <div className="grid grid-cols-2 gap-y-2 text-sm">
                                    {(!activeServices.includes('visas') && !activeServices.includes('flights')) && (
                                        <>
                                            <div className="text-slate-500">Departure City:</div>
                                            <div className="font-bold text-slate-700">{guestDetails.departureCity || 'Not specified'}</div>
                                            <div className="text-slate-500">Arrival Date:</div>
                                            <div className="font-bold text-slate-700">{guestDetails.arrivalDate || 'Not specified'}</div>
                                        </>
                                    )}
                                    <div className="text-slate-500">Services Requested:</div>
                                    <div className="font-bold text-slate-700 flex flex-wrap gap-2">
                                        {activeServices.length > 0 ? activeServices.map(s => (
                                            <span key={s} className="bg-white px-2 py-1 rounded border border-slate-200 text-xs capitalize">
                                                {s}
                                            </span>
                                        )) : 'None'}
                                    </div>
                            ...
                            <div className="space-y-3">
                                <h4 className="text-sm font-bold text-[#1E3A6D] uppercase tracking-wider">Services Breakdown</h4>
                                {journey.length === 0 ? (
                                    <p className="text-sm text-slate-500 italic">No specific services configured yet.</p>
                                ) : (
                                    <div className="space-y-2">
                                        {journey.map((item) => (
                                            <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 bg-white">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs
                                                    ${item.type === 'hotel' ? 'bg-sky-500' :
                                                        item.type === 'transport' ? 'bg-[#1c8f94]' :
                                                            item.type === 'visa' ? 'bg-[#24b3ba]' : 'bg-[#24B3BA]'}`}>
                                                    <i className={`fa-solid ${item.type === 'hotel' ? 'fa-hotel' :
                                                        item.type === 'transport' ? 'fa-bus' :
                                                            item.type === 'visa' ? 'fa-passport' : 'fa-plane'
                                                        }`}></i>
                        ...
                        <div className="p-6 border-t border-slate-100 bg-slate-50 flex gap-4">
                            ...
                            <button
                                onClick={() => {
                                    document.getElementById('submit-quotation-trigger')?.click();
                                    setShowSummary(false);
                                }}
                                className="flex-1 py-3 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-700 text-white font-bold hover:shadow-lg transition-all"
                            >
